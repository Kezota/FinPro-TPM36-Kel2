...
function saveTeamData() {
  const participantsContainer = document.querySelector(
    "#participants-page .accordion-container"
  );
  const teams = Array.from(
    participantsContainer.getElementsByClassName("accordion-item")
  ).map((team) => {
    const teamName = team.querySelector(".accordion-header p").textContent;
    const members = Array.from(
      team.querySelectorAll(".accordion-content ul li")
...
function loadTeamData() {
  ...
  if (savedData) {
    ...
    if (editContainer && teams.length > 0) {
      ...
      editContainer.innerHTML = members
        .map(
          (member) => `
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", function (event) {
    if (
      !birthInput.contains(event.target) &&
      !birthAccordContent.contains(event.target)
    ) {
      birthAccordContent.style.display = "none";
    }
  });
  ...
  if (form) {
    form.addEventListener("submit", function (event) {
      ...
      if (currentEditMember) {
        const numberPrefix =
          currentEditMember.querySelector("p").textContent.split(".")[0] + ". ";
        const teamName = currentEditMember.getAttribute("data-team");

        currentEditMember.querySelector("p").textContent =
          numberPrefix + newName;

        const teamNumber = numberPrefix.trim();
        const accordionItems = document.querySelectorAll(".accordion-item");
        accordionItems.forEach((item) => {
          const headerTeamName = item.querySelector(
            ".accordion-header p"
          ).textContent;
...
function editPage() {
  ...
  const members = Array.from(
    accordionItem.querySelectorAll(".accordion-content ul li")
...
function deleteTeam(event) {
  const button = event.target;
  const accordionItem = button.closest(".accordion-item");
  const teamName = accordionItem.querySelector(
    ".accordion-header p"
  ).textContent;
  ...
  const participantsTeam = Array.from(
    participantsContainer.getElementsByClassName("accordion-item")
  ).find(
    (item) => item.querySelector(".accordion-header p").textContent === teamName
  );
  if (participantsTeam) {
  ...
  const teamContainer = document.querySelector(
    "#team-page .accordion-container"
  );
  if (teamContainer) {
    const teamPageTeam = Array.from(
      teamContainer.getElementsByClassName("accordion-item")
    ).find(
      (item) =>
        item.querySelector(".accordion-header p").textContent === teamName
    );
    if (teamPageTeam) {
      teamPageTeam.remove();
    }
...
function sortTeams(sortBy) {
  let accordionContainer = document.querySelector(
    "#team-page .accordion-container"
  );
  ...
  clonedItems.sort((a, b) => {
    const teamNameA = a
      .querySelector(".accordion-header p")
...
function toggleBirthAccordion() {
  ...
  birthAccordContent.style.display =
    birthAccordContent.style.display === "none" ? "block" : "none";
}
